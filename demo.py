...
if __name__ == '__main__':
    ...
    with torch.cuda.amp.autocast(enabled=not args.no_amp) if device.type == 'cuda' else nullcontext():
        ...
        inference = streaming_inference(network, resource_manager, s2m_controller, fbrs_controller, config, device)
        
        # import mask/layer
        inference.on_import_mask("workspace/test/masks/0000000.png")

        #这个函数是前向传播核心函数
        inference.on_forward_propagation()

        # 这个函数是清除缓存
        inference.on_clear_memory()

        shutil.rmtree("workspace/video/images")
